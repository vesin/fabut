...
class FabutObjectAssert extends Assert {
	 ...
	 */
	protected boolean assertObjectWithProperties(
			final FabutReportBuilder report, final Object actual,
		...
		final Map<String, Object> fields = ReflectionUtil
				.getFieldsForAssertFromMethods(actual);
		boolean result = ASSERTED;
		for (final Entry<String, Object> field : fields.entrySet()) {

			final String fieldName = field.getKey();
			final ISingleProperty property = getPropertyFromList(fieldName,
					expectedProperties);
			try {
				if (property != null) {
					result &= assertProperty(fieldName, report, property,
	...
	boolean assertInnerProperty(final FabutReportBuilder report,
	 ...
	 *            use of this list is to remove the need for every field of
	 *            actual object to match fields of expected object, properties
	 *            in this list take priority over fields in expected object
	 * @return <code>true</code> can be asserted, <code>false</code> otherwise
	 */
	protected boolean assertObjects(final FabutReportBuilder report,
			final Object expected, final Object actual,
			final List<ISingleProperty> expectedChangedProperties) {

		final AssertPair assertPair = ConversionUtil.createAssertPair(expected,
				actual, types);
		final boolean assertResult = assertPair(EMPTY_STRING, report,
				assertPair, expectedChangedProperties, new NodesList());
		if (assertResult) {
			afterAssertObject(actual, false);
		}
		return assertResult;
	 ...
	 *         otherwise.
	 */
	boolean assertPair(final String propertyName,
			final FabutReportBuilder report, final AssertPair pair,
			final List<ISingleProperty> properties, final NodesList nodesList) {

		final ReferenceCheckType referenceCheck = checkByReference(report,
				pair, propertyName);
	 ...
	 *         otherwise.
	 */
	boolean assertSubfields(final FabutReportBuilder report,
			final AssertPair pair, final List<ISingleProperty> properties,
			final NodesList nodesList, final String propertyName) {

		report.increaseDepth(propertyName);

		boolean t = ASSERTED;
		final Map<String, Object> fieldsToBeAsserted = ReflectionUtil
				.getFieldsForAssertFromMethods(pair.getExpected());
		for (final Entry<String, Object> field : fieldsToBeAsserted.entrySet()) {
			try {
				final String fieldName = field.getKey();

				final ISingleProperty property = obtainProperty(
						field.getValue(), fieldName, properties);

				final Object actual = ReflectionUtil.getFieldValue(
	 ...
	 *         {@link AssertionError}.
	 */
	boolean assertPrimitives(final FabutReportBuilder report,
			final String propertyName, final AssertPair pair) {
		try {
	 ...
	 */
	boolean assertProperty(final String propertyName,
			final FabutReportBuilder report, final ISingleProperty expected,
			final Object actual, final String fieldName,
			final List<ISingleProperty> properties, final NodesList nodesList,
		...
		if (expected instanceof Property) {
			...
			final AssertPair assertPair = ConversionUtil.createAssertPair(
					expectedValue, actual, types, isProperty);
			return assertPair(propertyName, report, assertPair, properties,
					nodesList);
		}
	 ...
	 */
	List<ISingleProperty> removeParentQualification(
		...
		for (final ISingleProperty property : properties) {
			final String path = StringUtils.removeStart(property.getPath(),
					parentPrefix);
	 ...
	 */
	ISingleProperty obtainProperty(final Object field,
			final String propertyPath, final List<ISingleProperty> properties) {
		final ISingleProperty property = getPropertyFromList(propertyPath,
				properties);
		if (property != null) {
			return property;
		}
	 ...
	 */
	ISingleProperty getPropertyFromList(final String propertyPath,
			final List<ISingleProperty> properties) {
	 ...
	 */
	ReferenceCheckType checkByReference(final FabutReportBuilder report,
			final AssertPair pair, final String propertyName) {
	 ...
	 * @return
	 */
	List<ISingleProperty> extractPropertiesWithMatchingParent(
			final String parent, final List<ISingleProperty> properties) {
		final List<ISingleProperty> extracts = new LinkedList<ISingleProperty>();
		final Iterator<ISingleProperty> iterator = properties.iterator();
		while (iterator.hasNext()) {
			final ISingleProperty property = iterator.next();
			if (property.getPath().startsWith(parent + DOT)
					|| property.getPath().equalsIgnoreCase(parent)) {
	 ...
	 * @return
	 */
	boolean hasInnerProperties(final String parent,
			final List<ISingleProperty> properties) {
		for (final ISingleProperty property : properties) {
			if (property.getPath().startsWith(parent + DOT)) {
				return true;
