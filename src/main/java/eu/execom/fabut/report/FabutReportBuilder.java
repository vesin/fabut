...
/**
 * Report builder used for creating clean and readable reports. Its point is to
 * emphasize failed asserts so developer can track them easy.
 * 
 * @author Dusko Vesin
 * @author Nikola Olah
 * @author Bojan Babic
...
public class FabutReportBuilder {
	 ...
	 *            the actual size
	 */
	public void listDifferentSizeComment(final String propertyName,
			final int expectedSize, final int actualSize) {
		final String comment = String.format(
				"Expected size for list: %s is: %d, but was: %d", propertyName,
	 ...
	 * 
	 * <p>
	 * Example: <i>There was no property for field: name of class: User, with
	 * value: John</i>
	 * </p>
	 * 
	 * @param fieldName
	...
	public void noPropertyForField(final String fieldName, final Object field) {
		final String comment = String
				.format("There was no property for field:  %s of class:  %s, with value: %s",
	 ...
	 * <p>
	 * Example(asserted): <i>∞> id: expected not null property and field was not
	 * null</i>
	 * </p>
	 * <p>
	 * Example(assert fail): <i>■> id: expected not null property, but field was
	 * null</i>
	 * </p>
	 * 
	 * @param fieldName
	...
	public void notNullProperty(final String fieldName,
		...
		if (assertResult) {
			final String comment = String.format(
					"%s: expected not null property and field was not null",
					fieldName);
			addComment(comment, CommentType.SUCCESS);
		} else {
			final String comment = String.format(
					"%s: expected not null property, but field was null",
					fieldName);
			addComment(comment, CommentType.SUCCESS);
		}
	}
	 ...
	 * </p>
	 * <p>
	 * Example(asserted): <i>∞> endDate: expected null, but field was not
	 * null</i>
	 * </p>
	 * 
	 * 
	...
	public void nullProperty(final String fieldName, final boolean assertResult) {
		if (assertResult) {
			final String comment = String.format(
					"%s: expected null property and field was null", fieldName);
			addComment(comment, CommentType.SUCCESS);
		} else {
			final String comment = String.format(
					"%s: expected null property, but field was not null",
					fieldName);
			addComment(comment, CommentType.FAIL);
		}
	}
	...
	/**
	 * Reports result of of assertion with {@link NoneProperty}
	 * <p>
	 * Example(asserted): <i>∞> endDate: expected none and field was none</i>
	 * </p>
	 * <p>
	 * Example(asserted): <i>∞> endDate: expected none, but field was not
	 * none</i>
	 * </p>
	 * 
	 * @param fieldName
	 * @param assertResult
	 */
	public void noneProperty(final String fieldName, final boolean assertResult) {
		if (assertResult) {
			final String comment = String.format(
					"%s: expected none property and field was none", fieldName);
			addComment(comment, CommentType.SUCCESS);
		} else {
			final String comment = String.format(
					"%s: expected none property, but field was not none",
					fieldName);
			addComment(comment, CommentType.FAIL);
		}
	}
	 ...
	 * <p>
	 * Example(asserted): <i>∞> Property: address of class: User has good
	 * reference.</i>
	 * </p>
	 * <p>
	 * Example(assert fail): <i>■> Property: address of class: User has wrong
	 * reference.</i>
	 * </p>
	 * 
	 * @param fieldName
	...
	public void checkByReference(final String fieldName, final Object object,
		...
		if (asserted) {
			final String comment = String.format(
					"Property:  %s of class:  %s has good reference.",
					fieldName, object.getClass().getSimpleName());
			addComment(comment, CommentType.SUCCESS);
		} else {
			final String comment = String.format(
					"Property:  %s of class:  %s has wrong reference.",
					fieldName, object.getClass().getSimpleName());
			addComment(comment, CommentType.FAIL);
		}
	}
	...
	public void ignoredType(final AssertPair assertPair) {
		final String comment = String.format("Type  %s is ignored type.",
				assertPair.getExpected().getClass().getSimpleName());
		addComment(comment, CommentType.SUCCESS);
	}
	...
	public void assertingListElement(final String listName, final int index) {
		final String comment = String.format(
				"Asserting object at index [%d] of list %s.", index, listName);
		addComment(comment, CommentType.COLLECTION);
	}

	/**
	 * Reports failure when there is no matching entity in current snapshot for
	 * entity in before snapshot.
	 * <p>
	 * Example: <i>■> Entity User [id=1] doesn't exist in DB any more but is not
	 * asserted in test.</i>
	 * </p>
	 * 
	 * @param propertyName
	...
	public void noEntityInSnapshot(final Object entity) {
		final String comment = String
				.format("Entity %s doesn't exist in DB any more but is not asserted in test.",
						entity);
		addComment(comment, CommentType.FAIL);
	}

	/**
	 * Reports failure when entity in current snapshot doesn't have matching
	 * entity in before snapshot and isn't asserted in test.
	 * <p>
	 * Example: <i>■> Entity User [id=100] is created in system after last
	 * snapshot but hasnt been asserted in test..</i>
	 * </p>
	 * 
	 * @param entity
	 *            the entity
	...
	public void entityNotAssertedInAfterState(final Object entity) {
		final String comment = String
				.format("Entity %s is created in system after last snapshot but hasnt been asserted in test.",
						entity);
		addComment(comment, CommentType.FAIL);
	}
	 ...
	 * <p>
	 * Example: <i>■> There is no method: getName in actual object class:
	 * ShoppingCart (expected object class was: User).</i>
	 * </p>
	 * 
	 * 
	 * @param methodName
	...
	public void uncallableMethod(final Method method, final Object actual) {
		final String comment = String
				.format("There is no method: %s in actual object class: %s (expected object class was: %s).",
						method.getName(), actual.getClass(), method
	...
	public void asserted(final AssertPair pair, final String propertyName) {
		final String comment = String.format("%s: expected: %s and was: %s",
				propertyName, pair.getExpected(), pair.getActual());
		addComment(comment, CommentType.SUCCESS);
	}
	...
	public void assertFail(final AssertPair pair, final String propertyName) {
		final String comment = String.format("%s: expected: %s, but was: %s",
				propertyName, pair.getExpected(), pair.getActual());
		addComment(comment, CommentType.FAIL);
	}
	...
	public void idNull(final Class<?> clazz) {
		final String comment = String.format("Id of %s cannot be null",
				clazz.getSimpleName());
		addComment(comment, CommentType.FAIL);
	}
	...
	public void notExistingInSnapshot(final Object entity) {
		final String comment = String.format(
				"Entity: %s cannot be found in snapshot", entity);
		addComment(comment, CommentType.FAIL);
	}
	...
	public void notDeletedInRepositoy(final Object entity) {
		final String comment = String.format(
				"Entity: %s was not deleted in repository", entity);
		addComment(comment, CommentType.FAIL);
	}
	...
	public void noCopy(final Object entity) {
		final String comment = String.format(
				"Entity: %s cannot be copied into snapshot", entity);
		addComment(comment, CommentType.FAIL);
	}
	...
	public void excessExpectedMap(final Object key) {
		final String comment = String.format("No match for expected key: %s",
				key);
		addComment(comment, CommentType.FAIL);
	}
	...
	public void excessActualMap(final Object key) {
		final String comment = String
				.format("No match for actual key: %s", key);
		addComment(comment, CommentType.FAIL);
	}
