...
public class Fabut {
	...
	/**
	 * This method needs to be called in @Before method of a test in order for
	 * {@link Fabut} to work.
	 * 
	 * @param testInstance
	 *            the test instance
	...
	public static synchronized void beforeTest(final Object testInstance) {
		...
		case REPOSITORY_ASSERT:
			fabutAssert = new FabutRepositoryAssert(
					(IFabutRepositoryTest) testInstance);
			break;
		case UNSUPPORTED_ASSERT:
			throw new IllegalStateException(
					"This test must implement IFabutAssert or IRepositoryFabutAssert");
		default:
			throw new IllegalStateException("Unsupported assert type: "
					+ assertType);
		}
	}

	/**
	 * This method needs to be called in @After method of a test in order for
	 * {@link Fabut} to work.
	 */
	...
	/**
	 * Creates repository snapshot so it can be asserted with after state after
	 * the test execution.
	 */
	public static void takeSnapshot() {
		...
		if (assertType == AssertType.UNSUPPORTED_ASSERT) {
			throw new IllegalStateException(
					"Test must implement IRepositoryFabutAssert");
		}
	 ...
	 *            expected properties for asserting object
	 */
	public static void assertObject(final Object expected,
			final IProperty... properties) {
		checkValidInit();
	 ...
	 */
	public static void assertObjects(final String message,
		...
		if (!fabutAssert.assertObjects(report, expected, actual,
				fabutAssert.extractProperties(expectedChanges))) {
	 ...
	 *            property difference between expected and actual
	 */
	public static void assertObjects(final Object expected,
			final Object actual, final IProperty... expectedChanges) {
		checkValidInit();
	 ...
	 */
	public static void assertEntityWithSnapshot(final Object entity,
			final IProperty... expectedChanges) {
		checkValidInit();
	...
	/**
	 * Assert entity as deleted. It will fail if entity can still be found in
	 * snapshot.
	 * 
	 * @param entity
	 *            the entity
	...
	private static void checkIfRepositoryAssert() {
		...
		if (assertType != AssertType.REPOSITORY_ASSERT) {
			...
			throw new IllegalStateException(
					"Test class must implement IRepositoryFabutAssert");
		}
	}
	...
	private static void checkValidInit() {
		if (fabutAssert == null) {
			throw new IllegalStateException(
					"Fabut.beforeTest must be called before the test");
		}
	}
